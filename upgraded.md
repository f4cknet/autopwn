...
| `v4.0.2c` | **5 binary 6 关验收收尾（v4.0 GA 阻塞最后一步）**：跑 `autopwn -l Challenge/{rip,level3_x64,fmtstr1,canary,pie}` 五个 binary，对比 `verify_shell` 返 `(True, "uid=...")` 路径全部一致；canary 仍 PARTIAL（per `v4.0.2 备注` v3.1 pre-existing 限制，不阻 GA）；把 v4.0.2a/b 的修复纳入 `logs/v4.0/binary_<name>.log` 对比基线 | ⏳ | 1h | **依赖 v4.0.2a + v4.0.2b**：必须先修两个 bug 才能 6 关验收 |
