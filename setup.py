...
def main():
    ...
    print_status('info', f"Initializing AutoPwn setup environment...")
    time.sleep(0.5)
    ...
    print_status('success', f"AutoPwn framework successfully installed!")
    print_status('info', f"Ready for advanced PWN exploitation")
    print(f"\n{Colors.CYAN}    Usage: {Colors.WHITE}python autopwn.py -l <target_binary>{Colors.END}")
    print(f"{Colors.CYAN}    Help:  {Colors.WHITE}python autopwn.py --help{Colors.END}\n")

def setup_system_command():
    """Setup autopwn as a system command"""
    import shutil
    import stat
    
    # Get current script path
    current_dir = os.path.dirname(os.path.abspath(__file__))
    autopwn_script = os.path.join(current_dir, 'autopwn.py')
    ...
    try:
        ...
        print_status('process', f"Installing autopwn to {target_path}...")
        
        if os.name == 'posix':
            # For Linux/macOS, create a wrapper script
            wrapper_content = f"#!/usr/bin/env python3\n# AutoPwn System Command Wrapper\nimport sys\nimport os\nsys.path.insert(0, '{current_dir}')\nfrom autopwn import main\nif __name__ == '__main__':\n    main()\n"
            ...
            shutil.copy2(autopwn_script, target_path)
        
        print_status('success', f"AutoPwn installed as system command: {target_name}")
        print_status('info', f"You can now run 'autopwn' from anywhere in the terminal")
        ...
        print_status('info', "You can still run autopwn using: python autopwn.py")
...
if __name__ == '__main__':
    ...
    response = input(f"{Colors.CYAN}[◉]{Colors.END} Install autopwn as system command? (y/N): ").strip().lower()
    ...
    setup(
        ...
        author="qzdx_soc",
        author_email="soc@qzdx.example",
        url="https://github.com/f4cknet/autopwn",
        long_description="AutoPwn 是自研自动化二进制利用框架，基于开源项目 heimao-box/pwnpasi 改造（MIT 协议）。维护：衢州电信安全运营中心 (qzdx_soc)。",
